...
public class EntityPlayerSP extends AbstractClientPlayer {
	...
	public boolean sprintDisabledTick;
	...
	public void onLivingUpdate() {
		...
		if (this.mc.playerController.enableEverythingIsScrewedUpMode()) {
			...
			if (this.inPortal) {
				...
				if (this.timeInPortal == 0.0F) {
					this.mc.getSoundHandler().playSound(PositionedSoundRecord
							.func_147674_a(new ResourceLocation("portal.trigger"),
			...
			boolean isSprintDisabled = !ToggleSneak.TOGGLE_SPRINT.getValue();
			boolean canDoubleTap = ToggleSneak.DOUBLE_TAP_W.getValue();

			if (this.sprintDisabledTick) {
				this.setSprinting(false);
				this.updateSprint(false, false);
				this.sprintDisabledTick = false;
			}

			if (isSprintDisabled) {
				if (canDoubleTap && this.onGround && !var3 && this.movementInput.moveForward >= var2 &&
				    !this.isSprinting() && var4 && !this.isUsingItem() && !this.isPotionActive(Potion.blindness)) {
					if (this.sprintToggleTimer <= 0 && !this.mc.gameSettings.keyBindSprint.getIsKeyPressed()) {
						this.sprintToggleTimer = 7;
					} else {
						this.setSprinting(true);
						this.updateSprint(true, false);
					}
				}
				...
				boolean state = this.movementInput.sprint;

				if (var4 && !this.isUsingItem() && !this.isPotionActive(Potion.blindness) &&
				    !this.movementInput.sprintHeldAndReleased) {
					if (!canDoubleTap || !this.isSprinting()) {
						this.setSprinting(state);
					}
				}

				if (canDoubleTap && !state && this.onGround && !var3 && this.movementInput.moveForward >= var2 &&
				    !this.isUsingItem() && !this.isSprinting() && !this.isPotionActive(Potion.blindness)) {
					if (this.sprintToggleTimer == 0) {
						this.sprintToggleTimer = 7;
					} else {
						this.setSprinting(true);
						this.updateSprint(true, true);
						this.sprintToggleTimer = 0;
					}
				}
			}
			...
			if (this.isSprinting() &&
			    (this.movementInput.moveForward < var2 || this.isCollidedHorizontally || !var4)) {
				this.setSprinting(false);
				if (this.movementInput.sprintHeldAndReleased || isSprintDisabled ||
				    this.movementInput.sprintDoubleTapped || this.isRiding()) {
					this.updateSprint(false, false);
				}
			}

			if (ToggleSneak.FLY_BOOST.getValue() && this.capabilities.isFlying && this.capabilities.isCreativeMode &&
					this.mc.gameSettings.keyBindSprint.getIsKeyPressed()) {
				float flyBoost = ToggleSneak.FLY_BOOST_AMOUNT.getValue();
				this.capabilities.setFlySpeed(0.05F * flyBoost);

				if (this.movementInput.sneak) {
					this.motionY -= 0.15D * flyBoost;
				}
	...
	private void updateSprint(boolean toggle, boolean doubleTap) {
		if (this.movementInput != null) {
			this.movementInput.sprint = toggle;
			this.movementInput.sprintDoubleTapped = doubleTap;
		}
	}
	...
	public long lastSprintTrueTime;
	...
	public void setSprinting(boolean p_70031_1_) {
		...
		if (p_70031_1_) {
			this.lastSprintTrueTime = System.currentTimeMillis();
		}

		boolean debug = false;

		if (debug) {
			System.out.println("SPRINT " + p_70031_1_);
			try {
				throw new Exception("Printed");
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	public void playSound(String p_85030_1_, float p_85030_2_, float p_85030_3_) {
		this.worldObj
				.playSound(this.posX, this.posY - (double) this.yOffset, this.posZ, p_85030_1_, p_85030_2_, p_85030_3_,
						false);
	}
