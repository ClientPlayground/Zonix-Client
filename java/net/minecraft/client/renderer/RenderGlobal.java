...
public class RenderGlobal implements IWorldAccess {
	...
	private static AxisAlignedBB AABB_INFINITE = AxisAlignedBB
			.getBoundingBox(Double.NEGATIVE_INFINITY, Double.NEGATIVE_INFINITY, Double.NEGATIVE_INFINITY,
			...
			for (var10 = 0; var10 < this.renderChunksWide; ++var10) {
				for (int var14 = 0; var14 < this.renderChunksTall; ++var14) {
					for (int cz = 0; cz < this.renderChunksDeep; ++cz) {
						...
						this.worldRenderers[wri] = WrUpdates
								.makeWorldRenderer(this.theWorld, this.tileEntities, var10 * 16, var14 * 16, cz * 16,
	...
	public void renderEntities(EntityLivingBase p_147589_1_, ICamera p_147589_2_, float p_147589_3_) {
		...
		if (this.renderEntitiesStartupCounter > 0) {
			...
			TileEntityRendererDispatcher.instance
					.func_147542_a(this.theWorld, this.mc.getTextureManager(), this.mc.fontRenderer,
							this.mc.renderViewEntity, p_147589_3_);
			...
			if (pass == 0) {
				...
				double oldFancyGraphics = var17.lastTickPosY + (var17.posY - var17.lastTickPosY) * (double)
						p_147589_3_;
			...
			for (var25 = 0; var25 < var24.size(); ++var25) {
				...
				if (te && (var19 != this.mc.renderViewEntity || this.mc.gameSettings.thirdPersonView != 0 ||
				           this.mc.renderViewEntity.isPlayerSleeping()) && this.theWorld
						    .blockExists(MathHelper.floor_double(var19.posX), 0, MathHelper.floor_double(var19.posZ)
						    )) {
					++this.countEntitiesRendered;
			...
			for (var25 = 0; var25 < this.tileEntities.size(); ++var25) {
				...
				if (var29 == AABB_INFINITE || p_147589_2_.isBoundingBoxInFrustum(var29)) {
					...
					if (var30 == TileEntityChest.class) {
						Block var31 = this.theWorld
								.getBlock(var27.field_145851_c, var27.field_145848_d, var27.field_145849_e);
	...
	public String getDebugInfoRenders() {
		return "C: " + this.renderersBeingRendered + "/" + this.renderersLoaded + ". F: " + this
	...
	public String getDebugInfoEntities() {
		return "E: " + this.countEntitiesRendered + "/" + this.countEntitiesTotal + ". B: " + this
				.countEntitiesHidden +
		...
		for (int ix = 0; ix < this.renderChunksWide; ++ix) {
			...
			for (int iz = 0; iz < this.renderChunksDeep; ++iz) {
				...
				for (int iy = 0; iy < this.renderChunksTall; ++iy) {
					...
					WorldRenderer worldrenderer = this.worldRenderers[
							(iz * this.renderChunksTall + iy) * this.renderChunksWide + ix];
	...
	public int sortAndRender(EntityLivingBase player, int renderPass, double partialTicks) {
		...
		if (this.occlusionEnabled && this.mc.gameSettings.advancedOpengl && !this.mc.gameSettings.anaglyph &&
			...
			for (int switchStep = this.renderChunksWide; endIndex < this.countSortedWorldRenderers;
					...
					if (wr.skipAllRenderPasses()) {
						...
						if (wr.isInFrustum) {
							if (Config.isOcclusionFancy() && !wr.isInFrustrumFully) {
								...
								ARBOcclusionQuery
										.glBeginQueryARB(ARBOcclusionQuery.GL_SAMPLES_PASSED_ARB, wr.glOcclusionQuery);
	...
	private void checkOcclusionQueryResult(int startIndex, int endIndex, double px, double py, double pz) {
		for (int k = startIndex; k < endIndex; ++k) {
			...
			if (wr.isWaitingOnOcclusionQuery) {
				this.occlusionResult.clear();
				ARBOcclusionQuery
						.glGetQueryObjectuARB(wr.glOcclusionQuery, ARBOcclusionQuery.GL_QUERY_RESULT_AVAILABLE_ARB,
			...
			if (var6 == 0) {
				...
				for (var18 = 0; var18 < this.glRenderLists.size(); ++var18) {
					...
					for (var21 = 0; var21 < var17; ++var21) {
						if (this.allRenderLists[var21]
								.rendersChunk(var24.posXMinus, var24.posYMinus, var24.posZMinus)) {
							var20 = var21;
						}
					}
					...
					if (var20 < 0) {
						var20 = var17++;
						this.allRenderLists[var20]
								.setupRenderList(var24.posXMinus, var24.posYMinus, var24.posZMinus, var231, var13,
										var15);
					}
	...
	public void renderSky(float par1) {
		if (this.mc.theWorld.provider.dimensionId == 1) {
			...
			OpenGlHelper.glBlendFunc(770, 771, 1, 0);
			RenderHelper.disableStandardItemLighting();
			float[] var251 = this.theWorld.provider
					.calcSunriseSunsetColors(this.theWorld.getCelestialAngle(par1), par1);
			float var9;
			float var10;
			float var11;
			float var12;
				...
				if (Config.isCloudsFancy()) {
					...
					if (this.isFancyGlListClouds || this.cloudTickCounter >= this.cloudTickCounterGlList + 20) {
						...
						double cdx = this.mc.renderViewEntity.prevPosZ +
						             (this.mc.renderViewEntity.posZ - this.mc.renderViewEntity.prevPosZ) *
						             (double) par1;
		...
		if (!this.damagedBlocks.isEmpty()) {
			...
			while (var10.hasNext()) {
				...
				if (var12 * var12 + var14 * var14 + var16 * var16 > 1024.0D) {
					...
					Block var18 = this.theWorld
							.getBlock(var11.getPartialBlockX(), var11.getPartialBlockY(), var11.getPartialBlockZ());

					if (var18.getMaterial() != Material.air) {
						this.renderBlocksRg
								.renderBlockUsingTexture(var18, var11.getPartialBlockX(), var11.getPartialBlockY(),
	 ...
	 */
	public void drawSelectionBox(EntityPlayer par1EntityPlayer, MovingObjectPosition par2MovingObjectPosition, int
			par3,
	                             float par4) {
		if (par3 == 0 && par2MovingObjectPosition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
			...
			if (var6.getMaterial() != Material.air) {
				...
				drawOutlinedBoundingBox(
						var6.getSelectedBoundingBoxFromPool(this.theWorld, par2MovingObjectPosition.blockX,
								par2MovingObjectPosition.blockY, par2MovingObjectPosition.blockZ)
	...
	public void playRecord(String par1Str, int par2, int par3, int par4) {
		...
		if (par1Str != null) {
			...
			PositionedSoundRecord var8 = PositionedSoundRecord
					.func_147675_a(resource, (float) par2, (float) par3, (float) par4);
	...
	public EntityFX doSpawnParticle(String par1Str, double par2, double par4, double par6, double par8, double par10,
		...
		if (this.mc != null && this.mc.renderViewEntity != null && this.mc.effectRenderer != null) {
			...
			if (par1Str.equals("hugeexplosion")) {
				...
				if (Config.isAnimatedExplosion()) {
					this.mc.effectRenderer.addEffect(
							var21 = new EntityLargeExplodeFX(this.renderEngine, this.theWorld, par2, par4, par6, par8,
									par10, par12));
				}
			} else if (par1Str.equals("fireworksSpark")) {
				this.mc.effectRenderer.addEffect(
						var21 = new EntityFireworkSparkFX(this.theWorld, par2, par4, par6, par8, par10, par12,
	...
	/**
	 * Plays a pre-canned sound effect along with potentially auxiliary data-driven one-shot behaviour (particles,
	 * etc).
	 */
		...
		switch (par2) {
			...
			case 1005:
				if (Item.getItemById(par6) instanceof ItemRecord) {
					this.theWorld
							.playRecord("records." + ((ItemRecord) Item.getItemById(par6)).field_150929_a, par3, par4,
									par5);
				} else {
			...
			case 1007:
				this.theWorld
						.playSound((double) par3 + 0.5D, (double) par4 + 0.5D, (double) par5 + 0.5D, "mob.ghast" +
						                                                                             ".charge",
								10.0F, (var7.nextFloat() - var7.nextFloat()) * 0.2F + 1.0F, false);
				break;
			...
			case 1010:
				this.theWorld
						.playSound((double) par3 + 0.5D, (double) par4 + 0.5D, (double) par5 + 0.5D, "mob.zombie.wood",
								2.0F, (var7.nextFloat() - var7.nextFloat()) * 0.2F + 1.0F, false);
				break;

			case 1011:
				this.theWorld
						.playSound((double) par3 + 0.5D, (double) par4 + 0.5D, (double) par5 + 0.5D, "mob.zombie" +
						                                                                             ".metal",
								2.0F, (var7.nextFloat() - var7.nextFloat()) * 0.2F + 1.0F, false);
				break;
			...
			case 1014:
				this.theWorld
						.playSound((double) par3 + 0.5D, (double) par4 + 0.5D, (double) par5 + 0.5D, "mob.wither" +
						                                                                             ".shoot",
								2.0F, (var7.nextFloat() - var7.nextFloat()) * 0.2F + 1.0F, false);
				break;
			...
			case 1015:
				this.theWorld
						.playSound((double) par3 + 0.5D, (double) par4 + 0.5D, (double) par5 + 0.5D, "mob.bat.takeoff",
								0.05F, (var7.nextFloat() - var7.nextFloat()) * 0.2F + 1.0F, false);
				break;
			...
			case 2001:
				...
				if (var8.getMaterial() != Material.air) {
					this.mc.getSoundHandler().playSound(
							new PositionedSoundRecord(new ResourceLocation(var8.stepSound.func_150495_a()),
			...
			case 2002:
				...
				for (var16 = 0; var16 < 8; ++var16) {
					this.spawnParticle(var15, var9, var11, var13, var7.nextGaussian() * 0.15D, var7.nextDouble() *
					                                                                           0.2D,
				...
				for (var40 = 0; var40 < 100; ++var40) {
					...
					EntityFX var46 = this
							.doSpawnParticle(var20, var9 + var26 * 0.1D, var11 + 0.3D, var13 + var30 * 0.1D, var26,
									var28, var30);
			...
			case 2006:
				...
				if (var8.getMaterial() != Material.air) {
					...
					for (int var24 = 0; var24 < var23; ++var24) {
						...
						this.theWorld.spawnParticle("blockdust_" + Block.getIdFromBlock(var8) + "_" +
	...
	/**
	 * Starts (or continues) destroying a block with given ID at the given coordinates for the given partially
	 * destroyed
	 * value
	 */
	...
	private boolean isMovingNow(EntityLivingBase entityliving) {
		...
		return entityliving.isSneaking() ? true : ((double) entityliving.prevSwingProgress > maxDiff ? true :
				                                           (this.mc.mouseHelper.deltaX != 0 ? true :
						                                            (this.mc.mouseHelper.deltaY != 0 ? true : (Math
								                                                                                       .abs(
										                                                                                       entityliving.posX -
										                                                                                       entityliving
												                                                                                       .prevPosX) >
						                                                                                       maxDiff ?
								                                                                                       ...
								                                                                                       (Math.abs(
										                                                                                       entityliving.posY -
										                                                                                       entityliving.prevPosY) >
								                                                                                        maxDiff ?
										                                                                                        true :
										                                                                                        Math.abs(
												                                                                                        entityliving.posZ -
												                                                                                        entityliving.prevPosZ) >
										                                                                                        maxDiff)))));
	}
		...
		if (!te.hasWorldObj()) {
			...
			if (blockType == Blocks.enchanting_table) {
				...
				if (blockType != null && blockType != Blocks.beacon) {
					blockAabb = blockType
							.getCollisionBoundingBoxFromPool(te.getWorldObj(), te.field_145851_c, te.field_145848_d,
									te.field_145849_e);
