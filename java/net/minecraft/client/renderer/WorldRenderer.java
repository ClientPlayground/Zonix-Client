...
public class WorldRenderer {
	...
	public void setPosition(int par1, int par2, int par3) {
		if (par1 != this.posX || par2 != this.posY || par3 != this.posZ) {
			...
			this.rendererBoundingBox = AxisAlignedBB
					.getBoundingBox((double) par1, (double) par2, (double) par3, (double) (par1 + 16),
							(double) (par2 + 16), (double) (par3 + 16));
	...
	public void updateRenderer(EntityLivingBase p_147892_1_) {
		if (this.worldObj != null) {
			if (this.needsBoxUpdate) {
				...
				RenderItem.renderAABB(AxisAlignedBB
						.getBoundingBox((double) this.posXClip, (double) this.posYClip, (double) this.posZClip,
								(double) (this.posXClip + 16), (double) (this.posYClip + 16),
			...
			if (this.needsUpdate) {
				...
				byte var14 = 1;
				ChunkCacheOF chunkcache = new ChunkCacheOF(this.worldObj, xMin - var14, yMin - var14, zMain - var14,
								...
								for (int x = xMin; x < xMax; ++x) {
									...
									if (block.getMaterial() != Material.air) {
										...
										if (canRender) {
											...
											if (block.getRenderType() == 0 && x == viewEntityPosX &&
											    y == viewEntityPosY && z == viewEntityPosZ) {
	...
	protected void postRenderBlocks(int renderpass, EntityLivingBase entityLiving) {
		if (Config.isTranslucentBlocksFancy() && renderpass == 1 && !this.skipRenderPass[renderpass]) {
			this.vertexState = this.tessellator
					.getVertexState((float) entityLiving.posX, (float) entityLiving.posY, (float) entityLiving.posZ);
		}
