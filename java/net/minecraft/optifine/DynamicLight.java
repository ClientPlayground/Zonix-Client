...
public class DynamicLight
{
    ...
    public void update(RenderGlobal renderGlobal)
    {
        ...
        if (Math.abs(dx) > delta || Math.abs(dy) > delta || Math.abs(dz) > delta || this.lastLightLevel != lightLevel)
        {
            ...
            if (world != null)
            {
                Block setNewPos = world.getBlock(MathHelper.floor_double(posX1), MathHelper.floor_double(posY), MathHelper
            ...
            if (lightLevel > 0)
            {
                ...
                BlockPos chunkView = new BlockPos(MathHelper.bucketInt(pos.getX(), 16) * 16, MathHelper.bucketInt(pos.getY(), 16) * 16, MathHelper
		                                                                                                                                        .bucketInt(pos.getZ(), 16) * 16);
    ...
    {
        switch (DynamicLight.NamelessClass363811399.$SwitchMap$net$minecraft$util$EnumFacing[facing.ordinal()])
        {
            case 1:
