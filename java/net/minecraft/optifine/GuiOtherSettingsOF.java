	...
	public void initGui() {
		...
		for (int k = 0; k < j; ++k) {
			...
			if (!enumoptions.getEnumFloat()) {
				this.buttonList.add(new GuiOptionButton(enumoptions.returnEnumOrdinal(), x, y, enumoptions,
		...
		this.buttonList.add(new GuiButton(200, this.width / 2 - 100, this.height / 6 + 168 + 11, I18n
	...
	protected void actionPerformed(GuiButton guibutton) {
		if (guibutton.enabled) {
			if (guibutton.id < 200 && guibutton instanceof GuiOptionButton) {
				...
				this.mc.gameSettings.saveOptions();
				GuiYesNo scaledresolution = new GuiYesNo(this, "Reset all video settings to their default values?", "",
						9999);
	...
	private String[] getTooltipLines(String btnName) {
		return btnName.equals("Autosave") ? new String[]{"Autosave interval",
				...
				(btnName.equals("Lagometer") ? new String[]{"Lagometer",
						 ...
						 (btnName.equals("Debug Profiler") ? new String[]{"Debug Profiler",
						                                                  "  ON - debug profiler is active, slower",
						                                                  "  OFF - debug profiler is not active, " +
						                                                  "faster",
						                                                  "The debug profiler collects and shows " +
						                                                  "debug" +
						                                                  " information",
						                                                  "when the debug screen is open (F3)"} :
								  (btnName.equals("Time") ? new String[]{"Time",
								                                         " Default - normal day/night cycles",
								                                         " Day Only - day only",
								                                         " Night Only - night only",
								                                         "The time setting is only effective in " +
								                                         "CREATIVE mode",
								                                         "and for local worlds."} :
										   (btnName.equals("Weather") ? new String[]{"Weather",
										                                             "  ON - weather is active, " +
										                                             "slower",
										                                             "  OFF - weather is not active, " +
										                                             "faster",
										                                             "The weather controls rain, snow " +
										                                             "and thunderstorms.",
										                                             "Weather control is only possible" +
										                                             " for local worlds."} :
												    (btnName.equals("Fullscreen") ? new String[]{"Fullscreen",
												                                                 "  ON - use " +
												                                                 "fullscreen mode",
												                                                 "  OFF - use window " +
												                                                 "mode",
														     ...
														     (btnName.equals("Fullscreen Mode") ? new String[]{
																     "Fullscreen mode",
																     "  Default - use desktop screen resolution, " +
																     "slower",
																     "  WxH - use custom screen resolution, may be " +
																     "faster",
																     "The selected resolution is used in fullscreen " +
																     "mode (F11).",
