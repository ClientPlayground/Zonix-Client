...
public class CustomColorizer
{
    ...
    private static void readCustomPalettes(Properties props, String fileName)
    {
        ...
        for (int var20 = 0; var20 < var19.length; ++var20)
        {
            ...
            for (int ix = 0; ix < indexStrs.length; ++ix)
            {
                ...
                {
                    if (var21 != Block.getIdFromBlock(Blocks.grass) && var21 != Block
		                    .getIdFromBlock(Blocks.tallgrass) && var21 != Block.getIdFromBlock(Blocks.leaves) && var21 != Block
		                    .getIdFromBlock(Blocks.vine))
                    {
                        if (metadata == -1)
                        {
    ...
    {
        useDefaultColorMultiplier = foliageBirchColors == null && foliagePineColors == null && swampGrassColors == null && swampFoliageColors == null && blockPalettes == null && Config
		        .isSwampColors() && Config.isSmoothBiomes();
    }
    ...
    {
        return block.getMaterial() != Material.water ? block.colorMultiplier(blockAccess, x, y, z) : (waterColors != null ? (
		        Config.isSmoothBiomes() ? getSmoothColor(waterColors, blockAccess, (double)x, (double)y, (double)z, 3, 1) : getCustomColor(waterColors, blockAccess, x, y, z)) : (!Config
		        .isSwampColors() ? 16777215 : block.colorMultiplier(blockAccess, x, y, z)));
    }
    ...
    public static boolean updateLightmap(World world, float torchFlickerX, int[] lmColors, boolean nightvision)
    {
        ...
        else if (!FPSBoost.LIGHTING.getValue())
        {
            return false;
        }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            float gamma = Config.limitTo1(Config.getGameSettings().getGammaSetting());
